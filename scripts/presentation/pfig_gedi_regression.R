...
out_folder <- 'figures/presentation'
...
plot_effects <- function(effects_df, title='', subtitle='') {
  ggplot() +
    ...
    geom_pointrange(data=effects_df, aes(x = estimate, y = metric, color = effect, xmin = conf.low, xmax = conf.high),
                    ...
                    size = 0.1, linewidth = 0.4) +
    scale_color_manual(values = c("Burned" = "brown3", "Timing" = "forestgreen", "Interaction" = "dodgerblue2")) +
...
res_agbd <- get_mod_loop(p_agbd_fire, vars_agbd, t1='0', t2='1')
fig_agbd <- plot_effects(res_agbd, title='AGBD change due to fire') + 
  theme_transparent
ggsave(file.path(out_folder, 'change_agbd_fire.png'), fig_agbd, width = 3, height = 3, bg = 'transparent')
...
fig_fire <- plot_effects(res_fire) +
  theme_transparent
ggsave(file.path(out_folder, 'change_gedi_fire.png'), fig_fire, width = 4, height = 6, bg = 'transparent')
...
fig_recover <- plot_effects(res_recover) +
  theme_transparent
ggsave(file.path(out_folder, 'change_gedi_recover.png'), fig_recover, width = 4, height = 6, bg = 'transparent')
